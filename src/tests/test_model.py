...
pipeline = joblib.load(project_root / "models/mobile_data_consumption_pipeline.pkl")
...
for label, path in test_files.items():
    ...
    test_data = pd.read_parquet(project_root / path)
...
summary_df.to_csv(reports_dir / "scenario_evaluation_summary.csv", index=False)
print(f"\nSaved summary to {reports_dir / 'scenario_evaluation_summary.csv'}")
